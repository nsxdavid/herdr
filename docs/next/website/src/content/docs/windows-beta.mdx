...
The native Windows binary can act as a thin client for Linux and macOS Herdr hosts:

```powershell
herdr --remote workbox
```

Herdr does not use its control-socket connection reuse path on Windows. It still adds fallback SSH keepalives, but password authentication may prompt for more than one setup command. Key or Windows `ssh-agent` authentication is recommended. Windows remote hosts and remote clipboard-image bridging remain unsupported.
